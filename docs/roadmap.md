...
**UI-полировка (отдельная задача, из живого тестирования 2026-07-27):**
- Тоггл вкл/выкл сервиса без подписи/тултипа — неочевидно, что он правит `service.disabled`
  на уровне конфига (эффект применяется только при deploy), а не NAT-флаг на роутере.
- Нельзя редактировать существующий сервис (NAT enabled/notify/порты) — только add/delete.
- Нет индикатора «есть незадеплоенные изменения» (per-router hash-drift уже это умеет питать).
- Форма deploy по умолчанию с use-ssh-agent → первый status может отдать 502.
- Проверено на CHR (2026-07-27): uninstall из UI снёс все `mkpk-tt-*` (scripts/scheduler/filter/nat/
  address-list = 0) и глобалы `mkpkTt*`; остался лишь `mkpkPipeVal` — ручной probe-артефакт, не создаётся
...
- Релизная обвязка — **В РАБОТЕ**. Semver + тег `v0.1.0`; версия штампуется через `-ldflags`. GitLab CI
  (vet/build/test на `gitlab`-раннере). Дальше: готовые бинари в GitLab Releases на тег (CLI
  кросс-компиляцией; десктоп `.app`/DMG — на macOS-раннере).
- Стриминг прогресса деплоя — **СДЕЛАНО**. `POST /api/deploy/stream` отдаёт newline-delimited JSON
  (`{type:log|result|error}`): каждая SSH-команда транслируется живьём (`deploy.Client.OnLog`,
  `DeployOptions.OnLog`), UI показывает лог в реальном времени. Деградирует до батча, если writer не
  умеет flush (десктоп).
- Клиентский GUI (для получателей инвайта) — **АКТИВНАЯ ЗАДАЧА**. Решения зафиксированы: **нативный
  Swift 6 / SwiftUI**, macOS menu-bar app (NSStatusItem + кастомный NSPanel-попап), фоновый режим,
  иконка отражает состояние. Рантайм стука/проверки **переписывается на Swift** (CryptoKit SHA-512 +
  Network.framework), с golden-тест-векторами против Go-эталона; провижн-`mkpk` остаётся источником
  правды для формулы. Живёт в этом же репо (`client-macos/`). Синк импортированных инвайтов через
  **iCloud Keychain** (секреты шифруются). Дистрибуция — подписанный+нотаризованный DMG + Sparkle.
  Управление проектом по AGENTS.md-паттерну (см. OpenUsage, MIT); переиспользуемые скиллы/скрипты
  тянем локально в репо — либо адаптация (скиллы без лицензии), либо с атрибуцией (MIT-скрипты).
  Бриф на UI — [client-brief.md](client-brief.md).
- Revisit (не коммит): оценить нативный (Swift) admin-шелл для провижина ПОЗЖЕ, после того как клиент
  на Swift докажет направление. Сейчас провижн отлично живёт как Go-ядро + `serve` (браузер, любая ОС)
  + Wails-окно на маке; полный нативный rewrite большого админ-UI поверх Go-ядра — плохой ROI.
