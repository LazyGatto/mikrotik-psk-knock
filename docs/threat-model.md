...
## Провижн как общая инсталляция (сетевой режим)

По умолчанию `mkpk-provision serve` слушает только loopback и не требует пароля: конфиг лежит на машине
администратора и никуда не уходит. Это исходная модель.

Начиная с сетевого режима провижн можно поднять один на команду (например в Docker за обратным прокси) и
работать в него нескольким админам через веб. Это **осознанно расширяет trust zone**, и вот чем именно:

- В `mkpk.yaml` лежат SSH-креды ко всем роутерам, токены уведомлений и PSK всех юзеров. Компрометация
  такой инсталляции = компрометация всей инфраструктуры разом, а не одного юзера.
- Аутентификация — **один общий пароль без учётных записей**. Это сознательный размен: нет привязки
  действия к человеку (аудита) и нельзя отозвать доступ у одного админа иначе, чем сменив общий пароль
  для всех.
- Пароль хранится как argon2id-хеш в `mkpk-admin.json` (0600) рядом с конфигом — **не внутри** `mkpk.yaml`,
  который копируют, диффают и передают.
- Сессия — серверная, cookie `HttpOnly; SameSite=Strict; Secure` (когда TLS терминируется здесь или на
  прокси). CSRF закрывается обязательным заголовком `X-MKPK-Token`, привязанным к сессии: кросс-сайтом
  такой заголовок не отправить без CORS-preflight.
- Перебор пароля тормозится растущей задержкой после нескольких неудач.
- Смена пароля завершает все остальные сессии.

Что при этом **обязан** обеспечить администратор, иначе режим небезопасен:

- **TLS.** Инстанс говорит по HTTP и рассчитывает на обратный прокси. Он отказывается стартовать на
  не-loopback адресе без пароля, и отказывается без явного `--behind-proxy` / `MKPK_BEHIND_PROXY=1` —
  чтобы голый HTTP наружу нельзя было выставить по невнимательности.
- **Сетевую доступность.** Админка с SSH-доступом к роутерам не должна торчать в интернет без нужды:
  внутренняя сеть, VPN или хотя бы ограничение по адресам на прокси.
- **Права на том с конфигом.** Файлы пишутся 0600, но том/бэкапы — зона ответственности администратора.

Чего сетевой режим **не** даёт: аудита по людям, ролей и разграничения прав, отзыва доступа у одного
админа, шифрования конфига в покое.
