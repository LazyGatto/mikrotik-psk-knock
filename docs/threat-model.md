...
Раздача конфигурации конечному юзеру — через per-user invite-blob (см. [admin-app.md](admin-app.md)). Блоб
несёт всё состояние одного юзера (адрес роутера, его PSK, его сервисы/порты), поэтому его компрометация =
компрометация доступа этого юзера (не других — у каждого свой блоб/PSK). Требование: передача блоба только
в safe-env; по открытым каналам его слать нельзя.
