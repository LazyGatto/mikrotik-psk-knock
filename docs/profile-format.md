...
- `launch` — что GUI-клиент откроет после подтверждённого открытия порта:
  `rdp` | `ssh` | `http` | `https` | `vnc`, пусто — ничего. В инвайт уходит **тип**, а не
  командная строка: инвайты не подписаны, поэтому исполняемый текст в них не передаётся —
  клиент сам строит вызов из уже известных ему адреса роутера и порта (Windows: `mstsc /v:host:port`
  для `rdp`; macOS/Linux: `open rdp://…`). Пользователь может переопределить своей локальной
  командой в клиенте.
- `notify.*` — см. ниже.
