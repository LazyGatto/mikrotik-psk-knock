...
- **Второй запуск клиента больше не плодит копии.** Запуск ещё одного экземпляра
  выводит вперёд уже работающий: у Windows-клиента и админского десктопа — через
  штатный single-instance-замок Wails, у нативного macOS-клиента — проверкой
  по bundle id. Копии были не безобидны: каждая держала бы свои порты открытыми
  по своему расписанию, а две админки ещё и спорили бы за один конфиг.
- **Windows-клиент: иконка в трее исчезает при выходе.** Раньше она оставалась
  висеть, пока не наведёшь мышь: приложение завершалось, не сказав шеллу убрать
  иконку. Теперь выход идёт через явный демонтаж трея, на всех путях — и по
  крестику, и из меню трея.

## [0.16.0] — 2026-08-27

### Added

- **Windows-клиент умеет держать порт открытым** — как нативный macOS-клиент.
  Кнопка `⟳` у сервиса включает автоматический перестук незадолго до истечения
  окна роутера (за один bucket, но не меньше 15 и не больше 30 секунд), а в
  бейдже идёт обратный отсчёт — «открыто · ещё 42м». Перестук молчаливый:
  команду после стука он не запускает, иначе RDP открывался бы заново каждые
  сорок минут. Три неудачи подряд выключают удержание, чтобы клиент не долбился
  в недоступный роутер бесконечно. Кнопка недоступна, если в инвайте нет
  `allowed_timeout` — держать тогда нечем.
- **Windows-клиент сообщает о новой версии.** При запуске проверяет публичный
  релиз и, если вышла новее, показывает уведомление со ссылкой на страницу
  релиза. Обновление добровольное, оффлайн — молчит. Проверка вынесена в общий
  `internal/release` — админка спрашивает то же самое, и логика теперь одна.
...
- **Логи в контейнере стали пригодны для разбора**: у неуспешных запросов в
  журнал попадает текст ошибки, у каждого деплоя — итог отдельной строкой
  (`deploy apply GW-MSK: FAILED after 3.4s: …`), потому что поток деплоя всегда
  отвечает `200` и по коду ответа провал неотличим от успеха. Опрос страницы
  (раз в 20 с) не логируется, пока он успешен и быстр — `MKPK_DEBUG=1` включает
  и его. В строке запуска теперь видно версию.

### Fixed

- **Windows-клиент: крестик закрывает программу.** Раньше окно пряталось в трей
  (поведение macOS), и люди считали, что приложение закрыто, пока оно
  продолжало стучать. Теперь крестик спрашивает подтверждение системным
  диалогом и выходит, а в трей окно убирается кнопкой «свернуть».
- **Windows-клиент: UI перестал разъезжаться.** Кнопка «Постучать» уезжала за
  край окна (появлялась горизонтальная прокрутка), футер наезжал на последний
  сервис при прокрутке, «Удалить инвайт?» показывалось браузерным диалогом
  «Сообщение с wails.localhost», а счётчик писал «1 попыток». Строка сервиса
  теперь переносит блок кнопок целиком, футер в потоке, подтверждение —
  собственное окно, число попыток склоняется.

- **Передеплой больше не требует «нажать ещё раз»**. Первый `/import` после
  правок часто падал с `Script Error: 0` — сообщение, из которого не следует
  ничего. Теперь неудачный импорт повторяется один раз автоматически (`.rsc`
  идемпотентен по построению: он сносит всё своё до того, как создать заново),
  а в текст ошибки подкладывается хвост лога самого роутера — чтобы у следующего
  такого случая была причина, а не пожатие плечами.
- **При добавлении роутера теперь два адреса**: публичный (по нему стучатся
  клиенты) и SSH-адрес деплоя (по нему ходит сам провижн). Раньше поле было
  одно, и приходилось заводить роутер с локальным адресом, а потом править оба
  поля.
