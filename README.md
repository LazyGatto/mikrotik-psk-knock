...
- **Админка** `mkpk-provision` — конфиг, рендер RouterOS-скрипта, деплой по SSH, выдача
  ...
  (`mkpk-desktop`, нативное окно на Wails) — всё поверх одного ядра.

Криптографический рантайм полностью на стороне клиента; SSH — только канал развёртывания.
...
именем, бит исполняемости сохранён (`chmod +x` не нужен). Десктопный `.app` для macOS — отдельным
архивом.

> **macOS:** скачанные неподписанные бинари помечаются карантином Gatekeeper. Пока нет нотаризации,
> снять его вручную: `xattr -d com.apple.quarantine ./mkpk`
...
Рабочая ROS-only реализация с CLI, локальным веб-UI и десктопом, плюс SSH-провижининг; всё
проверено end-to-end на живом CHR (RouterOS 7.23.2). Текущая версия — **v0.3.0** (semver, пре-1.0).
Дальше по плану: стриминг прогресса деплоя, клиентский GUI для получателей инвайта, нотаризация
macOS-сборок. Подробности — в [docs/roadmap.md](docs/roadmap.md).
