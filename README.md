...
- **Автозапуск после стука (GUI-клиент, Windows)** — для каждого сервиса можно задать свою
  локальную команду (`start "" mstsc /v:{host}:{port}` и т.п.); она выполняется только после
  подтверждённого «открыто». Команду вводит сам пользователь и она хранится только на его машине:
  инвайты не подписаны, поэтому исполняемые строки в них принципиально не передаются.
