...
- **Автозапуск после стука (GUI-клиент)** — админ задаёт сервису тип приложения
  (`rdp` / `ssh` / `http` / `vnc`), он едет в инвайте, и клиент сам открывает нужное сразу после
  подтверждённого «открыто» — пользователю достаточно нажать «Постучать». В инвайте едет **тип**,
  а не команда: инвайты не подписаны, поэтому исполняемый текст в них принципиально не передаётся,
  а вызов клиент строит сам из адреса и порта. Пользователь может переопределить это своей
  локальной командой (`start "" myapp {host}:{port}`), которая хранится только на его машине.
...
- **Сетевой режим для команды** — тот же веб-UI можно поднять один на всех за обратным прокси: общий
  пароль (argon2id), серверные сессии, защита от одновременной правки конфига. Включается наличием
  пароля; локальный режим остаётся беспарольным. Ограничения — в [docs/threat-model.md](docs/threat-model.md).
...
**mkpk** — инструмент сетевой безопасности. Используйте его только на системах, которыми
владеете сами, или на тестирование которых у вас есть **явное разрешение**. Авторы не несут
ответственности за неправомерное использование или любой ущерб. Программа предоставляется
«как есть», без каких-либо гарантий — см. [LICENSE](LICENSE).
